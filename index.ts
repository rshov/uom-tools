...
export const LengthUOMs: LengthUOM[] = ["mm", "cm", "m", "in", "ft"] as const;
export const LengthDisplayFormats: LengthDisplayFormat[] = ["mm", "cm", "m", "in", "ft", "ft_in"] as const;
export const InchDisplayFormats: InchDisplayFormat[] = ["in", "in16", "in32", "in64"] as const;
export const VolumeUnitOfMeasures: VolumeUnitOfMeasure[] = ["mL", "oz"] as const;
 ...
 */
export function formatWholeFeet(
  length: number,
  uom: LengthUOM = "ft",
 ...
 */
export function formatFeetAndFractionalInches(
  length: number,
  uom: LengthUOM = "in",
  inchDisplay: InchDisplayFormat = "in16"
) {
 ...
 * @returns {String} A formatted string with the number of inches
 */
export function formatFeetAndDecimalInches(
  length: number,
  uom: LengthUOM = "in"
) {
  const totalInches = uom === "in" ? length : convert(length, uom).to("in");
  ...
  const feetStr = wholeFeet
    ? formatWholeFeet(wholeFeet, "ft", true)
    : undefined;
  return feetStr ? `${feetStr} ${inchesStr}` : inchesStr;
}
 ...
 * @returns {String} A formatted string with the number of inches
 */
export function formatDecimalInches(
  length: number,
  uom: LengthUOM = "in",
  showUnits?: boolean
) {
 ...
 */
export function formatCentimeters(
  length: number,
  uom: LengthUOM = "cm",
  showUnits?: boolean
) {
 ...
 */
export function formatFeetDecimal(
  length: number,
  uom: LengthUOM = "ft",
  showUnits?: boolean
) {
...
export function formatVolume(
  mL: number,
  unitOfMeasure: VolumeUnitOfMeasure,
  showUnits: boolean
) {
  if (unitOfMeasure === "mL") {
...
export function parseLength(
  input?: string,
  targetUnit: LengthUOM = "in",
  defaultUnit?: LengthUOM
) {
  if (!input) {
    return 0;
  }
 ...
 * @throws an error if the string format cannot be parsed
 */
export function parseInchesAndFeet(
  input?: string,
  defaultUnit: LengthUOM = "in"
) {
  if (!input) return 0;
